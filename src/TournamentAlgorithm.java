...
public class TournamentAlgorithm {
    ...
    // the URLs of questions images
    private ArrayList<String> imageUrls;
    
    // the queue stores active HITs
     ...
     */
    public TournamentAlgorithm(RequesterService service, ArrayList<String> imageUrls,
	    int k1, int k2, int f, int g, int o, double reward, boolean useTag,
	    int shuffleCount) {
	...
	for (int i = 0; i < this.imageUrls.size(); i++) {
		...
		this.activeHITQueue.add(new ImageSelectionHIT(this.service,
			...
			this.tag[0]++, this.k1, this.k2, this.f, this.g,
			this.o, this.reward));
	...
	if (!this.activeHITQueue.isEmpty()) {
		...
		if (hit.getHITStatus() == HITStatus.Reviewable) {
		    ...
		    this.activeHITQueue.remove(i);// NOTE: this changes the size
						  // of the ArrayList, don't
						  // forget to minus 1 at the
						  // end
	...
	if (!this.useTag) {
		...
		this.activeHITQueue.add(new ImageSelectionHIT(this.service,
			...
			level, tag[level]++, this.k1, this.k2, this.f, this.g,
			this.o, this.reward));
	    }
	}
		...
		while (!this.levelQueue.get(level).isEmpty()) {
			...
			this.activeHITQueue.add(new ImageSelectionHIT(service,
				...
				this.tag[level]++, this.k1, this.k2, this.f,
				this.g, this.o, this.reward));
		    }
		}
	...
	public void run() {
		...
		for (int level = 0; level < levelQueue.size(); level++) {
			...
			if (levelQueue.get(level).size() >= f) {
			    ...
			    activeHITQueue.add(new ImageSelectionHIT(service,
				    ...
				    tag[level], k1, k2, f, tempG, o, reward));
			}
		...
		ArrayList<String> urlsForANewHIT = new ArrayList<String>();
		for (int i = 0; i < num; i++) {
		    urlsForANewHIT.add(levelQueue.get(levelQueue.size() - 1)
			    .get(0).getUrl());
		    levelQueue.get(levelQueue.size() - 1).remove(0);
		}
		ImageSelectionHIT imgHit = new ImageSelectionHIT(service, alg,
			shuffleCount, urlsForANewHIT, 0, 0, k1, k2, num, 1, 1,
			reward);
		...
		levelQueue.add(new ArrayList<Item>());
		levelQueue.get(levelQueue.size() - 1).add(
			new Item(answers[0], levelQueue.size() - 1, 0));
